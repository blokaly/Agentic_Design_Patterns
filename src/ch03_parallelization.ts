...
const promptsData = promptsSchema.parse(yaml.load(fs.readFileSync('src/prompts/ch03_parallelization_prompts.yaml', 'utf8')));

// --- Define Independent Chains ---
const runParallelExample = async (llm: BaseChatModel, topic: string) => {
    // These three chains represent distinct tasks that can be executed in parallel.
    const summarizeChain = ChatPromptTemplate.fromMessages(promptsData.summarizePrompt.map((msg: Message) => [msg.type, msg.content]))
        .pipe(llm)
        .pipe(new StringOutputParser());

    const questionsChain = ChatPromptTemplate.fromMessages(promptsData.questionsPrompt.map((msg: Message) => [msg.type, msg.content]))
        .pipe(llm)
        .pipe(new StringOutputParser());

    const termsChain = ChatPromptTemplate.fromMessages(promptsData.termsPrompt.map((msg: Message) => [msg.type, msg.content]))
        .pipe(llm)
        .pipe(new StringOutputParser());
...
// 2. Define the final synthesis prompt which will combine the parallel results.
    const synthesisPrompt = ChatPromptTemplate.fromMessages(promptsData.synthesisPrompt.map((msg: Message) => [msg.type, msg.content]));
