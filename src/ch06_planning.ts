...
type ResponseOutput = ReasoningStep | WebSearchCall | CodeInterpreterCall | FinalReport;
...
async function runDeepResearch() {
    ...
    const promptsData = promptsSchema.parse(yaml.load(fs.readFileSync('src/prompts/ch06_planning_prompts.yaml', 'utf8')));
    ...
    try {
        ...
        const response: DeepResearchResponse = await (client as any).responses.create({
        ...
        const finalReportItem = response.output.find(item => item.type === "final_report") as FinalReport | undefined;
        ...
        if (annotations.length === 0) {
            ...
            annotations.forEach((citation: Annotation, i: number) => {
                const citedText = finalReportText.slice(citation.start_index, citation.end_index);
                console.log(`Citation ${i + 1}:`);
                console.log(`  Cited Text: "${citedText}"`);
                console.log(`  Title: ${citation.title}`);
                console.log(`  URL: ${citation.url}`);
                console.log(`  Location: chars ${citation.start_index}–${citation.end_index}\n`);
            });
        }
        console.log("\n" + "=".repeat(70) + "\n");
        ...
        const reasoningStep = response.output.find(item => item.type === "reasoning") as ReasoningStep | undefined;
        if (reasoningStep) {
        ...
        const searchStep = response.output.find(item => item.type === "web_search_call") as WebSearchCall | undefined;
        if (searchStep) {
        ...
        const codeStep = response.output.find(item => item.type === "code_interpreter_call") as CodeInterpreterCall | undefined;
        if (codeStep) {
