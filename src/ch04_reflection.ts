...
const promptsData = promptsSchema.parse(
  yaml.load(
    fs.readFileSync("src/prompts/ch04_reflection_prompts.yaml", "utf8"),
  ),
);

const runReflectionLoop = async (
  taskllm: BaseChatModel,
  reflectionllm: BaseChatModel,
) => {
  /**
  ...
  const messageHistory: BaseMessage[] = [
    new HumanMessage(promptsData.taskPrompt),
  ];
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    messageHistory.push(
      new HumanMessage(`Critique of the previous code:\n${critique}`),
    );
  }
...
const main = async () => {
  ...
  if (!config.OPENAI_API_KEY || !config.XAI_API_KEY) {
    console.log(
      "OPENAI_API_KEY or XAI_API_KEY is not set. Using FakeListChatModel for mocking.",
    );
