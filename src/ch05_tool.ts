...
const searchInformationTool = tool(
    ...
    },
    {
        ...
        schema: z.object({
            query: z.string().describe("The search query or question the user wants answered"),
        }),
    }
);
...
const runAgentWithTool = async (agentExecutor: any, query: string) => {
    ...
    try {
        const response = await agentExecutor.invoke({messages: [{role: "user", content: query}]});
